...
# --- 4. FUNCIÓN DE GUARDADO (Ahora sí refresca todo) ---
def registrar_seguimiento(dni, nombre, tipo, obs=""):
    try:
        ...
        st.success(f"✅ {tipo} registrado para {nombre}")
        st.balloons()
        time.sleep(1.5)
        
        # EL SECRETO DEL REFRESCO: Cambiamos la llave y limpiamos estados
        st.session_state.reset_key += 1
        st.session_state.pedir_obs = False
        st.rerun() # Esto limpia la pantalla para el siguiente DNI
...
if modo == "Marcación":
    ...
    with c_dni:
        # La key dinámica hace que la caja se limpie sola al refrescar
        dni = st.text_input("", key=f"dni_input_{st.session_state.reset_key}", label_visibility="collapsed")
    ...
    if dni:
        ...
        if not emp.empty:
            ...
            if ya_salio:
                ...
                with col2:
                    if st.button("🚶 PERMISO", disabled=(not ya_ingreso or ultimo == "SALIDA_PER_OUT"), use_container_width=True):
                        st.session_state.pedir_obs = True
                with col3:
                    if st.button("🔙 RETORNO", disabled=(ultimo != "SALIDA_PER_OUT"), use_container_width=True):
                ...
                if st.session_state.pedir_obs:
                    ...
                    if motive:
                        registrar_seguimiento(dni, nombre, "SALIDA_PER_OUT", obs=motive)
        else:
